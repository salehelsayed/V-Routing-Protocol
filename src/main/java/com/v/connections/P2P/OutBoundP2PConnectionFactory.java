...
public class OutBoundP2PConnectionFactory extends P2PConnectionFactory {
    private static OutBoundP2PConnectionFactory instance;

    private OutBoundP2PConnectionFactory(String sourceIP, int sourcePort, String destinationIP, int portStart, int portEnd) {
        // Private constructor to prevent instantiation
        super(  sourceIP,   sourcePort,    destinationIP,   portStart,   portEnd);
    }

    public static synchronized OutBoundP2PConnectionFactory getInstance(String sourceIP, int sourcePort, String destinationIP, int portStart, int portEnd) {
        if (instance == null) {
            instance = new OutBoundP2PConnectionFactory(sourceIP, sourcePort, destinationIP, portStart, portEnd);
        }
        return instance;
    }
